...
void MainWindow::on_action_evaluate_open_network_triggered()
{
    ...
    if(fileName.isEmpty())
        return;
    if(!load_from_file(evaluate.model,fileName.toUtf8().constData().c_str()))
    {
...
void MainWindow::on_action_evaluate_save_results_triggered()
{
    ...
    if(!evaluate.save_to_file(currentRow,file.toUtf8().constData().c_str()))
    {
...
void MainWindow::run_action(QString command)
{
    ...
    evaluate.proc_actions(command.toUtf8().constData().c_str(),param1,param2);
    on_eval_pos_valueChanged(ui->eval_pos->value());
}
