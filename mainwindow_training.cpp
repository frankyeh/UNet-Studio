...
void MainWindow::on_action_train_new_network_triggered()
{
    ...
    train.model = UNet3d(in_count,out_count,feature.constData());
...
{
    if(!load_from_file(train.model,fileName.constData().c_str()))
    {
...
void MainWindow::on_action_train_save_network_triggered()
{
    ...
    if(!fileName.isEmpty() && save_to_file(train.output_model,fileName.constData().c_str()))
    {
...
void MainWindow::on_actionApply_Label_Weights_triggered()
{
    ...
    if(default_weight.isEmpty())
        return;
    train.param.set_weight(default_weight.constData());
}
...
void MainWindow::on_train_start_clicked()
{
    ...
    for(auto& each : option->treemodel->name_data_mapping)
        train.param.options[each.first.constData()] = each.second->getValue().toFloat();
    ...
    for(size_t i = 0;i < image_list.size();++i)
    {
        if(!std::filesystem::exists(image_list[i].constData()) ||
           !std::filesystem::exists(label_list[i].constData()))
            continue;
        if(ui->list1->item(i)->checkState() == Qt::Checked)
        {
            train.param.image_file_name.push_back(image_list[i].constData());
            train.param.label_file_name.push_back(label_list[i].constData());
        }
    }
...
void MainWindow::on_list1_currentRowChanged(int currentRow)
{
    ...
    {
        tipl::shape<3> shape;
        if(!read_image_and_label(image_list[currentRow].constData(),label_list[currentRow].constData(),in_count,I1,I2,shape))
            I2.clear();
        if(!is_label)
        ...
        if(ui->train_view_transform->isChecked())
        {
            ...
            for(auto& each : option->treemodel->name_data_mapping)
                options[each.first.constData()] = each.second->getValue().toFloat();
