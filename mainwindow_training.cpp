...
void MainWindow::on_action_train_open_labels_triggered()
{
    ...
    for(int index = 0;index < label_list.size();++index)
        ...
        {
            std::string result;
            if(tipl::match_files(image_list[entry_index].toUtf8().constData(),label_list[entry_index].toUtf8().constData(),
                                  image_list[index].toUtf8().constData(),result) && QFileInfo(result.c_str()).exists())
            {
...
{
    if(!load_from_file(train.model,fileName.toUtf8().constData().c_str()))
    {
...
void MainWindow::on_action_train_save_network_triggered()
{
    ...
    if(!fileName.isEmpty() && save_to_file(train.output_model,fileName.toUtf8().constData().c_str()))
    {
